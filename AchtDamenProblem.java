...
public class AchtDamenProblem {
    ...
    public static void main(String[] args) {
        ...
        //Platziere(0, ChessTable, count);
        //nimmZuruck(1, 2, ChessTable);
        /*for(int i = 0; i < TD; i++){
            for(int j = 0; j < TD; j++){
    ...
    public static boolean bedroht(int Zeile, int Spalte, char[][] ChessTable){
        if(ChessTable[Zeile][Spalte] == 'Q' || ChessTable[Zeile][Spalte] == 'X'){
            return true;
        }else{
            return false;
        }
    ...
    public static void nimmZuruck(int z, int s, char[][] CT){
        for(int i = 0; i < 4; i++){ // TD = 4
            CT[z][i] = '.';
            CT[i][s] = '.';
        }
        for(int i = 0; (z + i) < 4 && (s + i) < 4; i++){
             CT[z + i][s + i] = '.';
        }
        for(int i = 0; (z - i) >= 0 && (s - i) >= 0; i++){
             CT[z - i][s - i] = '.';
        }
        for(int i = 0; (z - i) >= 0 && (s + i) < 4; i++){
             CT[z - i][s + i] = '.';
        }
        for(int i = 0; (z + i) < 4 && (s - i) >= 0; i++){
             CT[z + i][s - i] = '.';
        }   
    }
